...
// Тобто в jwtPayload ми зберігаємо всі дані, які були закодовані в JWT-токені, наприклад, userId, role,
// токени або інші дані про користувача.Потім ці дані можна використовувати для визначення прав доступу або для роботи з поточним користувачем у будь-якій частині програми.
// Тобто таким чином ми можемо перекидати вже перевірений об'єкт між middleware мідлварками та передавати в контролер
//
// якщо токен видавали ми і він валідний,то щоб не генерувати новий токен, ми цей розшифрований об'єкт
// (всі дані по юзеру, в т.ч значення яке міститься в access /  refresh токенах) який отримали дістаємо в наступній мідлварці чи контролері
// (в нашому випадку в контролері в запиті на delete) - це ми робимо для того, щоб мати можливість видалити самого себе,
...
// *tokenService.verifyToken використовуємо в auth.middleware.ts для перевірки валідності токенів, щоб виконати запит який нас цікавить (delete в нашому прикладі)
